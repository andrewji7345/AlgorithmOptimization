...
Generated evaluation plots and tables belong under `legacy/results/` and are ignored by Git.

Commands below use the package directory for evaluators and the release `src` directory for `cmsRun`, as indicated. Paths are relative to those directories, not to this guide. The old analyzer, cfi, and runnable cfg remain in their CMSSW locations.
...
Scripts in `legacy/run_scripts/interactive_scans/` run small grids directly with `cmsRun`. Invoke them from `CMSSW_15_0_19/src`, and edit their sample, loop ranges, and output paths as needed.
...
```bash
python3 legacy/evaluate_ntuplizer_pt_ak_ca.py \
  --input-dir /eos/uscms/store/user/aji/rootfiles_existingOptimization \
  --sample WbWb_4000_1000 --output-dir legacy/results/WbWb_4000_1000
```
...
python3 legacy/evaluate_ntuplizer_dc_mp_pt_ak_ca_th.py \
  --input-dir /eos/uscms/store/user/aji/rootfiles_existingOptimization \
  --output-dir legacy/results/evaluate_ntuplizer_dc_mp_pt_ak_ca_th --th-code 85
```
...
| `test/signalMCFiles/*.txt` | MiniAOD inputs grouped by decay channel and `(M_Suu, M_chi)`; the current shared sample lists remain in `test/signalMCFiles/`. |
| `legacy/debug/eventDebugReadout.txt` | Saved event-level debugging output. |
| `legacy/evaluate_ntuplizer.py` | Detailed single-ntuple old/new reconstruction diagnostics. |
| `legacy/evaluate_ntuplizer_pt_ak_ca.py` | Single-sample grid metrics, ranking CSV, heatmaps, and selected mass histograms. |
| `legacy/evaluate_ntuplizer_dc_mp_pt_ak_ca_th.py` | Main multi-decay/multi-mass evaluator with family comparisons, global ranking, regret, and coverage summaries. |
| `legacy/run_scripts/interactive_scans/run_ntuplizer_scan_*.sh` | Direct pT, radius, and thrust scan drivers. |
| `legacy/run_scripts/interactive_scans/evaluate_ntuplizer_scan_*.sh` | Wrappers for evaluating interactive scan outputs. |
| `legacy/run_scripts/condor_scans/submit_*.sh` | Validate, generate manifests, package/upload CMSSW, and submit scans. |
| `legacy/run_scripts/condor_scans/run_*.sh` | Worker scripts that run one point and copy its ROOT file to EOS. |
| `legacy/run_scripts/condor_scans/*.jdl` | HTCondor arguments, resources, logs, manifests, and rescue submissions. |
| `legacy/run_scripts/condor_scans/scan_parameters*.tsv` | Generated or saved job manifests. |
| `legacy/run_scripts/condor_scans/README*.md` | Legacy and two-threshold campaign notes. |
| `legacy/results/` | Generated CSV files and plots; ignored by Git. |
